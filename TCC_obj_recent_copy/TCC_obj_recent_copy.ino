...
uint8_t last_esq = 1;
uint8_t last_sel = 1;
// WN = 10 Hz; Z = 0.1; float e = 2.71828;                // -> Conferir se usa Hz ou rad/s
...
int final_ref_pos = 13000;  // Levar o carrinho até o final da trilha, por ex. 13300
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void limite() {
  // Serial.println("entrou na limite");
  if (flaghome == 0) {
    motor.frenagem();
    flaghome = 1;
  }
}  // vai sempre entrar na primeira iteração do programa, pois flaghome é setada como 0 inicialmente
...
void manual(void) {
  ...
  ref_pos = constrain(ref_pos, 100, 13300);
}
...
void zera(void) {
  switch (flaghome) {
    case 0:
      ...
      break;
    case 1:
      // encoder0Pos=19900; // Seta encoder para posição máxima
      encoder0Pos = 13300;  // Seta encoder para posição máxima
      // controle=-40;
      // aplica_controle();
      // ref_pos = 9950;
      ref_pos = 100;
    ...
    case 2:
      if (encoder0Pos <= 100) {
        ...
        flaghome = 0;
        Timer4.stop();
      }  // define qual o ponto de parada do motor na volta
         // if (encoder0Pos<=5000){
      break;
  }
}
...
void loop(void) {
    ...
    case 1:  // Selecionado
      switch (flagbtn) {
        ...
        case 1:             // Inicia Controle
                            // ------------------------------------------------------------------------------------- Primeira parte
          ref_pos = 13300;  // Levar o carrinho até o final da trilha, por ex. 19000
          Timer4.start();
        ...
        case 2:  // Modo Manual
          ...
          break;
        case 3:  // Modo Shaper
          if (encoder0Pos < I1-450){
            delay(tempos[0]);
            ref_pos = impulsos[0];
            // shaper();
            Timer4.start();
            // Serial.println(encoder0Pos, DEC);
          } else if (encoder0Pos < I2-1000){
            // Timer4.stop();
            delay(tempos[1]);
            ref_pos = impulsos[1];
            Timer4.start();
            Serial.println(encoder0Pos, DEC);
          } else if (encoder0Pos < 13300 - 1500){
            Timer4.stop();
            delay(tempos[2]);
            ref_pos = 13300;  // Levar o carrinho até o final da trilha, por ex. 19000
            Timer4.start();
  ...
  pos_carro = map(encoder0Pos, 0, 13300, 0, 535);
  pos_pen = map(encoder1Pos, 0, -2000, 0, 360);
  ang_pen = (pos_pen * pi) / 180;
